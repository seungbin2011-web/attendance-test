...
진입점: `personnel_test.html`(팀장·소장·관리자 통합 로그인), `admin_sql_test.html`(관리자·소장 전용 인원 및 출결등급 관리). `index_test.html`·`leader_test.html`은 첫 화면으로, `admin_test.html`은 SQL 관리자 화면으로 연결한다. 기존 운영 페이지/Apps Script는 변경하지 않는다.
...
수치는 최초 반입 기준이다. 일반 근로자 52행은 명부에 유지하지만 로그인 계정은 팀장·소장·관리자만 허용한다. 권한은 공개 JS의 역할 값이 아니라 서버 로그인 연결과 DB 함수에서 검사한다. 로그인 토큰은 같은 탭의 `sessionStorage`에만 보관해 페이지 이동과 새로고침을 지원한다.
